...
import { fetchMaterialById } from "@/components/material/api";
import { getMockCategoryById } from "@/components/material/mockData";
import {
...
export default async function MaterialDetailPage({ params }: PageProps) {
  ...
  const category = getMockCategoryById(material.categoryId);
    ...
    <div className="py-8" style={{ minHeight: "calc(100vh - 80px)" }}>
      <div className="mx-auto px-4 sm:px-6 lg:px-[100px]" style={{ maxWidth: "1440px", width: "100%" }}>
        <p className="text-sm text-gray-500 mb-4">
          ...
          {" / "}
          <Link href={ROUTES.PUBLIC_DATABASE} className="hover:text-gray-700">Veřejná databáze</Link>
        </p>
        ...
        {material.attachments && material.attachments.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                type="button"
                className="inline-flex items-center gap-2 px-3 py-2 rounded-md text-white text-sm font-medium transition-opacity hover:opacity-90"
                ...
                >
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <span className="text-xs text-gray-500">
                      {attachment.format} {attachment.sizeLabel}
                    </span>
                    <button
                      type="button"
