...
def load_learn_block_data(state: AgentState) -> AgentState:
    ...
    user: models.User | None = (
        ...
        .filter(models.User.user_id == state["user_id"])
        .first()
    )
    ...
    return {
        ...
        "ai_tone": user.ai_tone if user else "profesionální a neutrální",
        "ai_expression_level": user.ai_expression_level if user else "standardní srozumitelný jazyk",
    }