...
from api.enums import AssessmentSessionStatus
...
@router.get(
    "/course-assessments/{course_assessment_id}/sessions",
    operation_id="list_assessment_sessions",
    dependencies=[require_role("lector")],
)
def list_assessment_sessions(
    ...
    """Sessions dané konfigurace pro hodnotitele.

    Typicky `?status=awaiting_review` = fronta čekající na hodnocení.
    """
    return controllers.list_sessions_for_review(db, user, course_assessment_id, status)


@router.get(
    "/assessment-sessions/{session_id}/review",
    operation_id="get_assessment_review_detail",
    dependencies=[require_role("lector")],
)
def get_assessment_review_detail(
    session_id: int,
    db: SessionSqlSessionDependency,
    user: CurrentUser,
) -> ReviewDetailResponse:
    """Podklad pro hodnocení: odpovědi studenta, případné AI drafty (tahy)."""
    return controllers.get_review_detail(db, user, session_id)


@router.post(
    "/assessment-sessions/{session_id}/review",
    operation_id="submit_assessment_review",
    dependencies=[require_role("lector")],
)
async def submit_assessment_review(
    session_id: int,
    body: ReviewInput,
    db: SessionSqlSessionDependency,
    user: CurrentUser,
) -> SessionStateResponse:
    """Finální hodnocení session lektorem (session musí být awaiting_review)."""
    return await controllers.submit_review(db, user, session_id, body)
