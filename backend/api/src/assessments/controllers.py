...
def list_course_assessments(
    ...
    stm = (
        ...
        .order_by(models.CourseAssessment.position, models.CourseAssessment.course_assessment_id)
    )
    if not is_privileged:
...
def update_course_assessment(
    ...
    if body.position is not None:
        course_assessment.position = body.position
...
def _get_runnable_course_assessment(
    ...
    if not (course.is_active and course.status in ("approved", "archived")):
        raise HTTPException(
            status_code=400, detail="Kurz není aktivní a schválený"
        )
...
def list_sessions_for_review(
    ...
    user: models.User,
    course_assessment_id: int,
    status: AssessmentSessionStatus | None = None,
) -> list[SessionSummaryResponse]:
    """Sessions dané konfigurace pro hodnotitele (vlastník kurzu/superadmin)."""
    course_assessment = get_or_404(
        db,
        models.CourseAssessment,
        course_assessment_id,
    ...
    stm = (
        ...
        .order_by(models.AssessmentSession.created_at)
    )
    if status is not None:
        stm = stm.where(models.AssessmentSession.status == status)

    rows = db.scalars(stm).all()
    return [SessionSummaryResponse.model_validate(row) for row in rows]


def _get_reviewable_session(
    db: Session, user: models.User, session_id: int
) -> models.AssessmentSession:
    """Session pro hodnotitele — autorizace přes vlastnictví kurzu."""
    session = get_or_404(
        db, models.AssessmentSession, session_id, detail="Session nenalezena"
    )
    validate_owner_or_superadmin(session.course_assessment, user, "formát")
    return session
...
def get_review_detail(
    ...
    session = _get_reviewable_session(db, user, session_id)
    return ReviewDetailResponse(
        session_id=session.session_id,
        course_assessment_id=session.course_assessment_id,
        assessment_type_code=session.assessment_type_code,
        user_id=session.user_id,
        status=session.status,
        settings_snapshot=session.settings_snapshot,
        turns=[TurnResponse.model_validate(turn) for turn in session.turns],
    )


async def submit_review(
    db: Session, user: models.User, session_id: int, body: ReviewInput
) -> SessionStateResponse:
    session = _get_reviewable_session(db, user, session_id)
    ...
    if session.status != AssessmentSessionStatus.awaiting_review:
        raise HTTPException(
            status_code=409,
            detail=(
                "Session nečeká na hodnocení "
                f"(aktuální stav: {session.status.value})"
            ),
        )

    service = _get_service(db, session)
    try:
        result = await service.handle_review(body)
    except ValueError as e:
        db.rollback()
        raise HTTPException(status_code=400, detail=str(e)) from e
