...
class Course(TimestampMixin, SoftDeleteMixin, Base):
    ...
    assessments: Mapped[list[CourseAssessment]] = relationship(
        back_populates="course",
        primaryjoin="and_(Course.course_id==CourseAssessment.course_id, CourseAssessment.is_active==True)",
    )
...
class AssessmentType(TimestampMixin, SoftDeleteMixin, Base):
    """
    Katalog interakčních formátů (formulace otázek, sokratický dialog, rubrika...).
    Nový formát = nový řádek v katalogu + implementace v agents/assessments.
    Seeduje se ze seed.py, chování definuje registr v agents/assessments/base.py.
    """
...
class CourseAssessment(TimestampMixin, SoftDeleteMixin, Base):
    ...
    # Verze schématu settings — pro budoucí evoluci formátu nastavení
...
class AssessmentSession(TimestampMixin, SoftDeleteMixin, Base):
    ...
    # Sdílená hodnoticí pole — NULL u formátů bez verdiktu (dialog, formulace otázek)
...
class AssessmentTurn(TimestampMixin, Base):
    ...
    AI feedback, hodnocení lektora. Typově specifická data v `payload`.
    """
